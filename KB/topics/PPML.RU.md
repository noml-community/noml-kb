...
# PPML (Privacy Preserving ML), PET, конфиденциальные вычисления
...
* [Мероприятия ИОИПР](#мероприятия-иоипр)
...
**Что это такое.** Речь о наборе технологий, которые позволяют нескольким сторонам (компаниям, ведомствам) совместно анализировать данные или обучать модели так, чтобы каждая сторона получала пользу от чужих данных, но не получала доступа к самим этим данным в открытом виде. Вместо схемы «отдай мне свою базу — я её проанализирую» появляется схема «мы вместе считаем результат, оставляя детальные данные каждый у себя». Разные методы решают эту задачу по-разному: гомоморфное шифрование позволяет вычислять прямо над зашифрованными данными; SMPC разбивает вычисление между несколькими сторонами так, что ни одна из них не видит данные других целиком; криптоанклавы (TEE) исполняют код в аппаратно изолированной области процессора, недоступной даже владельцу сервера; федеративное обучение отправляет модель к данным (а не данные к модели) и агрегирует только обновления весов; дифференциальная приватность добавляет контролируемый шум, чтобы результат нельзя было связать с конкретным человеком; синтетические данные заменяют реальные записи сгенерированными, которые сохраняют статистические свойства и зависимости оригинала, но не являются данными реальных людей или компаний.

**Чем это не является.** Конфиденциальные вычисления — это не то же самое, что шифрование данных при передаче или хранении (TLS, шифрование диска): та защита работает от внешнего наблюдателя или похитителя носителя, но не мешает стороне, которая обрабатывает данные, увидеть их в открытом виде. Это также не «анонимизация» в смысле простого удаления имён и ID из таблицы — такие данные обычно легко деанонимизируются через комбинацию оставшихся признаков. И это не единая технология с единственной «правильной» реализацией: методы сильно отличаются по гарантиям (криптографические против статистических), по производительности (от почти бесплатного FL до на порядки более медленного FHE) и по модели угроз, которую они закрывают, — выбор всегда компромиссный и зависит от того, от кого именно защищаются данные: от внешнего злоумышленника, от оператора инфраструктуры или от самого партнёра по коллаборации.

**Пример: Совместная скоринговая проверка между банками.** Два банка хотят проверить, не является ли новый заёмщик клиентом, уже находящимся в чёрном списке или имеющим избыточную долговую нагрузку в другом банке, — но ни один из банков не готов передать другому свою клиентскую базу. Протокол SMPC позволяет вычислить результат пересечения или агрегированный показатель риска таким образом, что каждый банк вносит свою часть вычисления и получает только итоговый ответ («да/нет» или скорректированный скоринговый балл), не видя данные другого банка.

**Пример: Федеративное обучение диагностической модели между больницами.** Несколько клиник хотят обучить модель, распознающую редкую патологию по медицинским снимкам, но объём данных в каждой отдельной клинике недостаточен для качественной модели, а вывоз данных пациентов за пределы клиники запрещён регуляторикой. При федеративном обучении модель поочерёдно "приезжает" в каждую клинику, дообучается на локальных данных, и на центральный сервер отправляются только обновлённые веса модели, а не сами снимки или истории болезни.

**Пример: Data Clean Room для рекламной аналитики.** Ритейлер и медиаплощадка хотят оценить эффективность рекламной кампании — сколько людей увидели рекламу и затем совершили покупку, — но ни один из них не готов передать другому полный список своих клиентов. Data Clean Room (обычно на основе TEE или SMPC) позволяет сопоставить обезличенные идентификаторы и получить агрегированную статистику пересечения аудиторий, не раскрывая сторонам конкретные списки пользователей.

**Пример: Синтетические тестовые данные для разработки и аутсорса.** Банку нужно передать датасет транзакций внешней команде разработчиков или консультантам для построения модели, но передавать реальные данные клиентов нельзя. Генератор синтетических данных строит новый датасет, который сохраняет распределения, корреляции и редкие паттерны (например, характерные признаки мошеннических транзакций) исходных данных, но каждая запись сгенерирована и не соответствует реальному человеку.

**Пример: Публикация статистики с дифференциальной приватностью.** Ведомству нужно опубликовать агрегированную статистику (например, распределение доходов по районам) так, чтобы результат нельзя было использовать для установления дохода конкретного человека, даже если у атакующего есть почти вся остальная информация о нём. Механизм дифференциальной приватности добавляет к результату контролируемый случайный шум, откалиброванный так, что присутствие или отсутствие любого одного человека в датасете почти не влияет на итоговую публикуемую цифру, — это математически доказуемая гарантия, а не эвристика.

**Дополнительно.** На практике методы редко используются по одному: федеративное обучение часто дополняют дифференциальной приватностью (чтобы даже обновления весов не позволяли восстановить обучающий пример) или SMPC (чтобы сервер агрегации не видел индивидуальные обновления участников), а криптоанклавы нередко служат "несущей конструкцией" для Data Clean Room или для узла агрегации в федеративном обучении. Поэтому в разделах ниже методы разнесены по отдельности для удобства навигации, но в реальных решениях они, как правило, комбинируются с учётом конкретной модели угроз, требований регулятора и допустимой просадки производительности.

## Мероприятия ИОИПР

* [СОЗВОН: Проблемы адаптации технологий конфиденциальных вычислений и PET](/2026/2026-07-15-CALL-PET/README.md), 2026. [YouTube](https://youtu.be/j2evpz4vG0w) \| [Дзен](https://dzen.ru/video/watch/6a618da07033a635243f8d7d) \| [RuTube](https://rutube.ru/video/5e8ba169a4d6b5250a6bc480baebbc77/) \| [Файл](https://disk.yandex.ru/i/RUq2BoAs8bmsdA) *(~1 час 50 минут)*;

* [СОЗВОН: Дмитрий Маслов, Михаил Фатюхин, Денис Афанасьев, Евгений Попов, Роман Постников, Павел Снурницын, Federated Learning и конфиденциальный анализ данных](/2025/2025-05-07-CALL-FL/README.md), 2025. [YouTube](https://youtu.be/JpApLfde38I) \| [Дзен](https://dzen.ru/video/watch/6820f8ad299a2d186a9f0b8b) \| [RuTube](https://rutube.ru/video/4feeead3035a80bc8d3bc405d281fab3/) *(~1 час 25 минут)*;
...
Материалы организованы согласно такой верхнеуровневой классификация методов PPML:
...
Про текуще положение дел можно также почитать в саммари наших круглых столов:
* [Созвон: Проблемы адаптации технологий конфиденциальных вычислений и PET](/2026/2026-07-15-CALL-PET/README.md#саммари-встречи-денис-афанасьев), 2025 *(~5 минут)*
* [Созвон: Federated Learning и конфиденциальный анализ данных](/2025//2025-05-07-CALL-FL/README.md#мысли-про-fl-и-ppml-по-мотивам-созвона), 2025 *(~2 минуты)*.
...
ПНСТ АБД:
* [Серия ПНСТ «Синтез данных»](https://lib.rubda.ru/best/seriya-pnst-sintez-dannyh.html)

## Ландшафты решений

Ландшафты открытых и коммерческих решений по основным подходам PPML, в скобках указан год составления/актуализации ландшафта. Ландшафты по своей природе быстро устаревают (стартапы закрываются, пивотятся или поглощаются, у больших вендоров появляются новые продуктовые линейки), в заголовке в скобках указан год составления/пристальной актуализации, а там, где при очередной сверке (2026) обнаружились существенные изменения, добавлены пометки "UPD".
...
UPD 2026:
* Intel SGX прекращена в потребительских процессорах (Core 11-го/12-го поколения), но продолжает поддерживаться на серверных Intel Xeon Scalable; для конфиденциальных ВМ Intel теперь продвигает [Intel TDX (Trust Domain Extensions)](https://www.intel.com/content/www/us/en/developer/tools/trust-domain-extensions/overview.html);
* Наследник ARM TrustZone для серверных/облачных сценариев — [ARM CCA (Confidential Compute Architecture)](https://www.arm.com/architecture/security-features/arm-confidential-compute-architecture);
* Крупные облака предлагают готовые продукты поверх этих технологий: [Azure Confidential Computing](https://azure.microsoft.com/en-us/solutions/confidential-compute), [Google Cloud Confidential Computing / Confidential Space](https://cloud.google.com/confidential-computing), в дополнение к уже указанному AWS Nitro Enclaves.
* Появился отдельный класс "GPU Confidential Computing" — конфиденциальные вычисления на уровне GPU (NVIDIA H100/H200/Blackwell), актуально именно для защищённого инференса и дообучения LLM;
...
UPD 2026:
* [Flower](https://github.com/adap/flower) вырос из проекта Оксфорда в отдельную компанию Flower Labs (бывшая Adap — см. также в списке коммерческих платформ ниже) и на сегодня, судя по всему, наиболее активно развиваемый и популярный open-source FL-фреймворк, с коммерческими надстройками (Flower Intelligence, FlowerLLM) и партнёрствами (например, с Apheris, март 2025).
* [Substra](https://docs.substra.org/en/stable/), изначально разработанный Owkin, с 2023 передан в Linux Foundation AI & Data Foundation как самостоятельный open-source проект.
...
UPD 2026:
* Adap стала компанией [Flower Labs](https://flower.ai/) — тем самым бизнесом, что стоит за открытым фреймворком Flower из списка выше; де-факто это уже не отдельная строчка, а коммерческая надстройка над Flower;
* DynamoFL по факту пивотнулась: теперь называется Dynamo AI и позиционируется как платформа AI-безопасности и compliance для LLM/агентов (guardrails, hallucination detection), а не как FL/DP-платформа для обучения моделей — исходная строчка в ландшафте PPML уже не вполне соответствует профилю компании.
* Owkin сместил позиционирование от «FL-стартапа для медицины» к более широкой «agentic AI для биологии» платформе для диагностики и разработки препаратов, но федеративная инфраструктура (Substra) остаётся ядром технологии.
...
UPD 2026:
* Gretel в марте 2025 приобретён NVIDIA (команда и технология влились в генеративный AI-стек NVIDIA); как самостоятельный независимый вендор Gretel больше не существует;
* Statice был приобретён Anonos, технология интегрирована в платформу Anonos Data Embassy;
* Synthesized сместил фокус с генерации синтетических данных для обучения ML-моделей на автоматизацию тестовых данных для QA/software testing (AI-native test data automation); тема смежная, но по факту это уже другая ниша;
